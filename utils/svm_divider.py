...
class svm_divider(object):
    def __init__(self, doc_vec_train, svm_model_by_industry_dir_road, doc_vec_validation, doc_vec,
                 doc_vec_kernel, predict_result_road):
        self.doc_vec_train = doc_vec_train
        self.svm_model_by_industry_dir_road = svm_model_by_industry_dir_road
    ...
    def svm_model_train(self):
        ...
        for i in range(len(category_list_by_industry)):
            for j in range(i + 1, len(category_list_by_industry)):
                ...
                category_group_name = get_group_dir(category_name1, category_name2,
                                                    self.svm_model_by_industry_dir_road)
                ...
                svc.fit(matrix, label)


                pickle_store(svc, self.svm_model_by_industry_dir_road + category_group_name + "/svm_model.pckl")
                ...
                acc_score = svc.score(matrix_validaion, label_validaion)
                print("当前二分类器的验证集上的准确率为", acc_score)

    def svm_predict(self):
        print("正在计算多分类器的预测结果")
        if 'svm_predict_result.pckl' not in os.listdir(self.predict_result_road):
            category_list_by_industry = get_category_list_by_insudtry()
            original_label = []
            predict_label = []
            predict_result = {}
            model_dict = model_read(self.svm_model_by_industry_dir_road)
                    ...
                    for m in range(len(kernel_category_list)):
                        for n in range(m + 1, len(kernel_category_list)):
                            if int(kernel_category_list[m]) < int(kernel_category_list[n]):
                                svm_model = model_dict[kernel_category_list[m] + "_" + kernel_category_list[n]]
                                predict_to_be_chosed.append(svm_model.predict([docvec]))
                            else:
                                svm_model = model_dict[kernel_category_list[n] + "_" + kernel_category_list[m]]
                                predict_to_be_chosed.append(svm_model.predict([docvec]))
            ...
            pickle_store(predict_result, self.predict_result_road + "svm_predict_result.pckl")
        else:
            predict_result = pickle_read(self.predict_result_road + "svm_predict_result.pckl")
